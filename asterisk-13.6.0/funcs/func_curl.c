...
static int acf_curl_file_helper(struct ast_channel *chan, const char *cmd, char *info, char *buf, struct ast_str **input_str, ssize_t len)
{
	int ret = -1;
	AST_DECLARE_APP_ARGS(args,
		AST_APP_ARG(url);
...
static int acf_curl_file_exec(struct ast_channel *chan, const char *cmd, char *info, char *buf, size_t len)
{
	return acf_curl_file_helper(chan, cmd, info, buf, NULL, len);
}

static int acf_curl2_file_exec(struct ast_channel *chan, const char *cmd, char *info, struct ast_str **buf, ssize_t len)
{
	return acf_curl_file_helper(chan, cmd, info, NULL, buf, len);
}
static struct ast_custom_function acf_curl_file = {
        .name = "CURL_FILE",
        .synopsis = "Retrieves the contents of a URL",
        .syntax = "CURL_FILE(url[,post-data])",
        .desc =
        "  url       - URL to retrieve\n"
        "  post-data - Optional data to send as a POST (GET is default action)\n",
        .read = acf_curl_file_exec,
        .read2 = acf_curl2_file_exec,
};
...
static int unload_module(void)
{
	...
	res |= ast_custom_function_unregister(&acf_curl_file);
...
static int load_module(void)
{
	...
	res |= ast_custom_function_register(&acf_curl_file);
